...
import org.smartregister.eusm.layer.DigitalGlobeLayer;
import org.smartregister.eusm.layer.MapBoxLayer;
...
public class AppMapHelper {
    ...
    private final Location indexCaseLocation = null;

    private GeoJsonSource indexCaseSource;

    private final float radius = Float.valueOf(getGlobalConfig(INDEX_CASE_CIRCLE_RADIUS_IN_METRES, DEFAULT_INDEX_CASE_CIRCLE_RADIUS_IN_METRES.toString()));

    private LineLayer indexCaseLineLayer;

    private Feature circleFeature;

    public static void addCustomLayers(@NonNull Style mapboxMapStyle, Context context) {
...
//        mapboxMapStyle.addImage(MOSQUITO_COLLECTION_ICON, icon);
...
//        mapboxMapStyle.addImage(POTENTIAL_AREA_OF_TRANSMISSION_ICON, icon);
...
//        indexCaseLocation = (new AppMappingHelper()).getCenter(indexCase.geometry().toJson());
//
//        try {
