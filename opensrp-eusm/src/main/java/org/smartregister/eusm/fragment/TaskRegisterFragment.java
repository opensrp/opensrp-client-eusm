...
public class TaskRegisterFragment extends BaseDrawerRegisterFragment implements TaskRegisterFragmentContract.View, BaseDrawerContract.DrawerActivity {
    ...
    private final RefreshRegisterReceiver refreshRegisterReceiver = new RefreshRegisterReceiver();
    ...
    public void setupViews(View view) {
        ...
        if (getActivity() != null) {
            interventionTypeTv.setText(
                    getActivity().getIntent().getStringExtra(AppConstants.TaskRegister.INTERVENTION_TYPE));
        }
        ...
        TaskFilterParams filterParams = (TaskFilterParams) getActivity().getIntent().getSerializableExtra(AppConstants.Filter.FILTER_SORT_PARAMS);
...
//        getPresenter().searchServicePoints(filterString);
    }
...
//            intent.putExtra(org.smartregister.eusm.util.Constants.Filter.FILTER_SORT_PARAMS, taskFilterParams);
//        } else if (StringUtils.isNotBlank(getSearchView().getText())) {
//            intent.putExtra(org.smartregister.eusm.util.Constants.Filter.FILTER_SORT_PARAMS, TaskFilterParams.builder().searchPhrase(searchView.getText().toString()).build());
//        }
    ...
    public Location getLastLocation() {
        if (getActivity() != null && getActivity().getIntent().getExtras() != null) {
            return getActivity().getIntent().getExtras().getParcelable(AppConstants.TaskRegister.LAST_USER_LOCATION);
        } else {
            return null;
        }
    }
    ...
    protected void initializePresenter() {
        presenter = new TaskRegisterFragmentPresenter(this, AppConstants.TaskRegister.VIEW_IDENTIFIER);
    ...
    protected void onViewClicked(View view) {
        ...
        if (AppConstants.Intervention.TASK_RESET_INTERVENTIONS.contains(details.getTaskCode())
...
//        }
    ...
    public void setJsonFormUtils(AppJsonFormUtils jsonFormUtils) {
...
//        intent.putExtra(AppConstants.Filter.FILTER_SORT_PARAMS, filterParams);
...
////                    .businessStatusList(Arrays.asList(org.smartregister.eusm.util.Constants.BusinessStatus.NOT_VISITED, org.smartregister.eusm.util.Constants.BusinessStatus.NOT_SPRAYED, org.smartregister.eusm.util.Constants.BusinessStatus.PARTIALLY_SPRAYED, org.smartregister.eusm.util.Constants.BusinessStatus.SPRAYED))
////                    .sortOptions(R.array.task_sort_options_namibia);
////        }
////        intent.putExtra(org.smartregister.eusm.util.Constants.Filter.FILTER_CONFIGURATION, builder.build());
//        getActivity().startActivityForResult(intent, AppConstants.RequestCode.REQUEST_CODE_FILTER_TASKS);
    }
...
//            }
        ...
        } else if (requestCode == AppConstants.RequestCode.REQUEST_CODE_FILTER_TASKS && resultCode == RESULT_OK && data.hasExtra(AppConstants.Filter.FILTER_SORT_PARAMS)) {
            TaskFilterParams filterParams = (TaskFilterParams) data.getSerializableExtra(AppConstants.Filter.FILTER_SORT_PARAMS);
            getPresenter().filterServicePoints(filterParams);
        }
    }
    ...
    public void onResume() {
        super.onResume();
        if (getContext() != null) {
            IntentFilter filter = new IntentFilter(AppConstants.Action.STRUCTURE_TASK_SYNCED);
