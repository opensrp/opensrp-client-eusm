...
import org.smartregister.eusm.interactor.LoginInteractor;
...
public class LoginPresenter extends BaseLoginPresenter implements BaseLoginContract.Presenter {
    ...
    public void processViewCustomizations() {
        try {
            String jsonString = getJsonViewFromPreference(AppConstants.VIEW_CONFIGURATION_PREFIX + AppConstants.CONFIGURATION.LOGIN);
            if (jsonString == null) {
                return;
            }

            ViewConfiguration loginView = EusmApplication.getJsonSpecHelper().getConfigurableView(jsonString);
