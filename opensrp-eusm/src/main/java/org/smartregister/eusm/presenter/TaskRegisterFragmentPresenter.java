...
public class TaskRegisterFragmentPresenter extends BaseFormFragmentPresenter implements TaskRegisterFragmentContract.Presenter {
    ...
    private final WeakReference<TaskRegisterFragmentContract.View> view;

    private final String viewConfigurationIdentifier;

    private final ConfigurableViewsHelper viewsHelper;
    ...
    private final boolean isActionClicked = true;
        ...
        if (!location.equals(lastLocation)) {
            if (lastLocation == null && tasks == null) {//tasks not yet retrieved from db
                recalculateDistance = true;
                    ...
                    location.distanceTo(lastLocation) >= AppConstants.REFRESH_MAP_MINIMUM_DISTANCE) {
...
//            if (AppConstants.Intervention.CASE_CONFIRMATION.equals(details.getTaskCode())) {
...
//                            AppConstants.Intervention.BEDNET_DISTRIBUTION.equals(details.getTaskCode()) ||
//                            AppConstants.Intervention.REGISTER_FAMILY.equals(details.getTaskCode())) ||
...
//    @Override
//    public void searchServicePoints(String searchText) {
...
//    }
    ...
    public void filterServicePoints(TaskFilterParams filterParams) {
...
//        Set<String> filterInterventionUnitTasks = Utils.getInterventionUnitCodes(filterParams.getCheckedFilters().get(AppConstants.Filter.INTERVENTION_UNIT));
...
//                if (AppConstants.Intervention.IRS.equals(getTaskDetails().getTaskCode()) && Country.NAMIBIA.equals(BuildConfig.BUILD_COUNTRY)) {
////                    formInteractor.findSprayDetails(Constants.Intervention.IRS, getStructure().getId(), formJSON);
//                } else {
...
//        }
        ...
        if ((AppConstants.Intervention.IRS.equals(getTaskDetails().getTaskCode()))
                && !Task.TaskStatus.READY.name().equals(getTaskDetails().getTaskStatus())) { // no event for READY tasks
            interactor.findLastEvent(getTaskDetails().getTaskEntity(), AppConstants.SPRAY_EVENT);
        } else {
