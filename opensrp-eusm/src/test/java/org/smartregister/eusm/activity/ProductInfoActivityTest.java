...
public class ProductInfoActivityTest extends BaseActivityUnitTest {
    ...
    public void setUp() {
        ...
        controller = Robolectric.buildActivity(ProductInfoActivity.class, intent).create().start();
    ...
    @Test
    public void testOnOffsetChangedShouldToggleAppBarTitle() {
        AppBarLayout appBarLayoutMock = mock(AppBarLayout.class);
        activity.onOffsetChanged(appBarLayoutMock, 0);
        assertTrue(ReflectionHelpers.getField(activity, "appBarTitleIsShown"));

        activity.onOffsetChanged(appBarLayoutMock, 1);
        assertFalse(ReflectionHelpers.getField(activity, "appBarTitleIsShown"));
    }
